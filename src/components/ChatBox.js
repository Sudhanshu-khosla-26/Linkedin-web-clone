...
import { getUserChatMessageAPI } from "../actions";
...
const ChatBox = (props) => {
  ...
  useEffect(() => {
    onValue(dbRef(db, `Users/${ChatId}/UserInfo`), (snapshot) => {
  ...
  // props.UserChatMessages.sort((a, b) =>
  //   (a.timestamp.seconds) - (b.timestamp.seconds) , "sorted")
  ...
  const sendMessage = () => {
    set(dbRef(db, `Users/${props.User.uid}/Chat/${ChatId}/${id}/`), {
      ImageUrl: ImageUrl,
      Message: input,
      SharedPost: "",
      fileInfo: {
    ...
    set(dbRef(db, `Users/${ChatId}/Chat/${props.User.uid}/${id}/`), {
      ImageUrl: ImageUrl,
      Message: input,
      SharedPost: "",
      fileInfo: {
    ...
    set(
      dbRef(
        db,
        `Users/${props.User.uid}/UserConnections/${ChatId}/ConnectData/Chat/${id}/`
      ),
      {
        ImageUrl: ImageUrl,
        Message: input,
    ...
    set(
      dbRef(
        db,
        `Users/${ChatId}/UserConnections/${props.User.uid}/ConnectData/Chat/${id}/`
      ),
      {
        ImageUrl: ImageUrl,
        Message: input,
  ...
  const removeMessage = (messageId) => {
    ...
    remove(dbRef(db, `Users/${props.User.uid}/Chat/${ChatId}/${messageId}/`));
    
    remove(dbRef(db, `Users/${ChatId}/Chat/${props.User.uid}/${messageId}/`));
    
    remove(
      dbRef(db,`Users/${props.User.uid}/UserConnections/${ChatId}/ConnectData/Chat/${messageId}/`));
    
      remove(
      dbRef(db,`Users/${ChatId}/UserConnections/${props.User.uid}/ConnectData/Chat/${messageId}/`));
  }
  ...
  return ChatId === ":ChatId" ? (
    ...
    Reciver && (
      <Container key={ChatId}>
        <div className="chat_titleInfo_bar">
          <div className="userside">
            <span>{Reciver.Name}</span>
            <span className="descrition">{Reciver.Description}</span>
          </div>
        ...
        <div
          style={Attachement ? { height: "131px" } : null}
          className="middlesection"
        >
          <div className="chat_UserInfo">
            <img src={Reciver.profilephoto} alt="" />

            <span>
              {Reciver.Name}
              <span>(He/Him) · 1st</span>
            </span>
            <div className="decription">{Reciver.Description}</div>
          </div>
          <div className="chatmessagesbox">
            ...
            {props.UserChatMessages.length > 0 &&
              props.UserChatMessages.map((item , key) => (
